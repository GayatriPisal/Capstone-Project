...
<head>
	<title>Edulearn Education Category Bootstrap Responsive Website Template | Register Form :: W3layouts</title>
	<!-- Meta tag Keywords -->
...
<body>
			...
			<div class="sub-main-w3 pt-md-4">
				<form action="" method="post">
					<div class="form-style-agile form-group">
						...
						<input placeholder="Your Names" class="form-control" name="name" id="name" type="text" required="">
					</div>
					<div class="form-style-agile form-group">
						<label>
							Email
					...
					<div class="form-style-agile form-group">
						...
						<input placeholder="Password" class="form-control" name="password" id="password1" type="password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>>
					</div>
					<div class="form-style-agile form-group">
						<label>
							Confirm Password
						...
						<input placeholder="Confirm Password" class="form-control" name="confirm_password" id="password2" type="password" required="">
					</div>
					<!-- switch -->
	...
	$fName=$Name[0];
		$lName=$Name[1];
		$sqlCheck = "SELECT email from user where email ='$_POST[email]'";// AND user_password = ''";
		$result = mysqli_query($conn,$sqlCheck);
		if(!mysqli_num_rows($result)){
			echo " if condition";
			$sql="INSERT into user(firstName,lastName,email,user_password) VALUES('$fName','$lName','$_POST[email]','$_POST[password]')";
			$result = mysqli_query($conn,$sql);
			$message = "Registered Successfully!!!";
		echo "<script type='text/javascript'>alert('$message');</script>";
		}else{
			echo "else condition";
			$message = "You already registered, try log in!!!";
		echo "<script type='text/javascript'>alert('$message');</script>";
		}
	}
  ?>
